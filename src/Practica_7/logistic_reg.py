...
def run_student_sim() -> None:
    ...
    plt.clf()
    plot_linear_data(J_history, range(num_iters + 1),
                     'muestreo1_cost.png', 'log')
    plot_linear_data(predict_history, range(num_iters + 1),
                     'muestreo1_accuracy.png', 'log')
...
def run_chip_sim() -> None:
    ...
    plot_linear_data(predict_history, range(
        num_iters + 1), 'muestreo2_accuracy.png')
    print(f'Cost: {J_history[-1]}, w: {w}, b: {b}')
    print(f'Accuracy rate: {predict_history[-1] * 100}%')
